...
Deno.serve(async (req) => {
  try {
    ...
    const { incidentId, comment, assignedToUserId } = await req.json();
    ...
    const updated = await base44.asServiceRole.entities.SystemIncident.update(incidentId, updateData);

    await base44.asServiceRole.entities.AuditLog.create({
      action: 'INCIDENT_ACKNOWLEDGED',
      performedByUserId: user.id,
      performedByName: user.full_name,
      details: JSON.stringify({ incidentId, incidentType: incident.incidentType, assignedToUserId: updateData.assignedToUserId })
    });
